function [newImg,dataArray] = snrMapUpd(img, sectionHW)
...
for r = 1:rows
    for c = 1:cols
        ...
        [sf,gof] = fit([x,y],z_alt,f,'StartPoint',z(I1,I2),'problem',sectionHW);
        coefficients = coeffvalues(sf);
        
        %sets paramaters to remove outliers, these can be altered
        %Currently sets the SNR to zero if:
        % 1) center of fit lies over twice the halfwidth away from the
        % current location, OR
        % 2) the standard devation of the fit is greater than an upper bound
        % reasonable number
            ...
            signal_c = r;
            signal_r = c;
            area_half_width = 2.355*sectionHW;
            ...
            for r_t = 1:rows
                for c_t = 1:cols
                    img_gauss(r_t,c_t) = coefficients(1)*exp( -( (r_t-r)^2 / (2*sectionHW^2) + (c_t-c)^2/(2*sectionHW^2)));
                end
            end
            ...
            if isnan(signal/noise) || isinf(signal/noise)
                newImg(r,c) = 0;
            else
            ...
            dataArray(r,c,:) = [signal-mu_noise,noise,coefficients(1)];
    end
end
toc